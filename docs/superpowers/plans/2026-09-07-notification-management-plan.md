...
- 通知队列最多 8 条；每个事件默认冷却 300 秒；默认只有短信事件启用。
...
**Interfaces:**
- `notification_event_id()`、`notification_event_from_id()`、`notification_rules_defaults()`。
- `notification_expand_template()` 替换通用模板变量。

- [ ] 写失败测试：事件 ID 映射、默认规则、短信和通用模板替换。
...
- [ ] 增加 `notification_rules` 表及出厂重置清理。
